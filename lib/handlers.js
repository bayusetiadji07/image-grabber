...
const { sniffImageType, jelaskanBukanGambar } = require('./sniff');
...
async function handleImage(req, res, url) {
    ...
    const msg =
      upstream.status === 429
        ...
        : upstream.status === 401 || upstream.status === 403
          ? `Akses ke gambar ini ditolak (HTTP ${upstream.status}) — kemungkinan hanya bisa diambil setelah login.`
          : `Gagal mengambil gambar (HTTP ${upstream.status}).`;
    throw Object.assign(new Error(msg), {
      status: 502,
      code: upstream.status === 401 || upstream.status === 403 ? 'perlu-login' : '',
    });
  }

  const contentType = upstream.headers.get('content-type') || '';

  // Jalur cepat: server sudah menyatakan ini gambar, alirkan apa adanya.
  if (/^image\//i.test(contentType)) {
    const headers = { 'Content-Type': contentType, 'Cache-Control': 'private, max-age=600' };
    const len = upstream.headers.get('content-length');
    if (len) headers['Content-Length'] = len;
    if (download) {
      headers['Content-Disposition'] = `attachment; filename*=UTF-8''${encodeURIComponent(
        ensureExtension(sanitizeFilename(name || guessNameFromUrl(target)), contentType)
      )}`;
    }
    res.writeHead(200, headers);
  ...
  // Tipe meragukan (octet-stream, text/html, kosong): periksa isinya sendiri
  // supaya halaman login tidak pernah tersimpan sebagai berkas gambar.
  const buf = Buffer.from(await upstream.arrayBuffer());
  const sniffed = sniffImageType(buf);
  if (!sniffed) {
    throw Object.assign(new Error(jelaskanBukanGambar(buf, contentType, upstream.url || target)), {
      status: 502,
      code: 'bukan-gambar',
    });
  }
  ...
  if (download) {
    headers['Content-Disposition'] = `attachment; filename*=UTF-8''${encodeURIComponent(
      ensureExtension(sanitizeFilename(name || guessNameFromUrl(target)), sniffed)
    )}`;
  }
  res.writeHead(200, headers);
  res.end(buf);
...
async function handleZip(req, res) {
  ...
  // Gambar yang diambil lewat tab asal dititipkan sebagai data URI, jadi body
  // permintaan ZIP bisa jauh lebih besar daripada permintaan biasa.
  const body = await readPayload(req, 32 * 1024 * 1024);
    ...
    const fetched = await Promise.all(
      batch.map(async (item) => {
        try {
          ...
          if (!r.ok) {
            throw new Error(
              r.status === 401 || r.status === 403
                ? `HTTP ${r.status} — akses ditolak, kemungkinan perlu login`
                : `HTTP ${r.status}`
            );
          }
          ...
          // Jangan pernah memasukkan halaman login/galat ke dalam ZIP —
          // berkas seperti itu tampak "rusak" saat dibuka pengguna.
          if (!/^image\//i.test(ct)) {
            const sniffed = sniffImageType(buf);
            if (!sniffed) throw new Error(jelaskanBukanGambar(buf, ct, r.url || item.url));
            return { item, buf, contentType: sniffed };
          }
          return { item, buf, contentType: ct };
        } catch (err) {
